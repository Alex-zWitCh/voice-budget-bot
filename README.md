...
Telegram → voice/text → local STT / Groq → DeepSeek JSON → SQLite → PNG / XLSX.
...
- **Отчёты**: PNG-диаграммы за 30 дней и ежемесячный пакет (PNG + Excel .xlsx) в
  основной валюте.
- **/ask — аналитика естественным языком**: задайте вопрос текстом или голосом;
  бот анализирует только доступные личные и семейные записи и отвечает текстом
  либо инфографикой.
...
`/ask` позволяет задавать вопросы к уже накопленным данным естественным языком
(текст или голос) и получает ответ текстом либо PNG-инфографикой.

Примеры:

- «Сколько я в среднем трачу на кафе в месяц?»
- «Сравни расходы на продукты за последние три месяца.»
- «На что наша семья потратила больше всего в августе?»
- «Покажи динамику расходов на продукты за год.»

Можно попросить и перечень операций построчно — «введи построчно все мои
траты на алкоголь» — тогда бот покажет каждую запись (дата, сумма, категория,
описание) и итог.

Режим полностью read-only: отдельное SQLite-соединение открывается в режиме
`mode=ro` + `PRAGMA query_only=ON`, а доступ ограничен личными и семейными
записями текущего пользователя. Арифметика и агрегация выполняются сервером;
LLM используется только для интерпретации вопроса и формулировки.

Для диагностики каждый запрос `/ask` пишется в таблицу `ask_requests`
(вопрос, план, сколько строк найдено, формат ответа, время и ошибка;
финансовые строки не сохраняются). Отключить можно `ASK_HISTORY_ENABLED=false`.
...
- графические отчеты считаются в основной валюте пользователя (по сохранённым
  курсам), подробный Excel-файл показывает исходную и основную валюты одновременно;
...
- автоматический ежемесячный пакет отчетов каждое 1-е число: диаграмма расходов, диаграмма доходов и Excel-таблица (.xlsx) со всеми записями за прошлый календарный месяц;
...
- `/ask` — аналитический вопрос к своим и семейным данным (текст или голос).
- `/cancel` — отменить активный диалог /ask.
...
- Каждое 1-е число бот автоматически отправляет пользователям отчетный пакет за прошедший календарный месяц: PNG-диаграмму расходов, PNG-диаграмму доходов и Excel-таблицу (.xlsx) со всеми записями периода. В таблице: дата, тип операции (доход/расход), категория, сумма и валюта траты, сумма в основной валюте, параметры конвертации (из какой валюты, сумма, курс), описание и полная расшифровка сообщения.
